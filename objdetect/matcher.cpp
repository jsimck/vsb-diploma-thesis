...
void Matcher::generateFeaturePoints(std::vector<Template> &templates) {
    ...
    for (size_t i = 0; i < iSize; i++) {
        ...
        cherryPickFeaturePoints(edgePoints, edgePoints.size() / criteria.featurePointsCount, criteria.featurePointsCount, t.edgePoints);
        cherryPickFeaturePoints(stablePoints, stablePoints.size() / criteria.featurePointsCount, criteria.featurePointsCount, t.stablePoints);
...
void Matcher::extractFeatures(std::vector<Template> &templates) {
    ...
    for (size_t i = 0; i < iSize; i++) {
        ...
        for (int j = 0; j < criteria.featurePointsCount; j++) {
...
int Matcher::testGradients(uchar gradient, Window &window, cv::Mat &sceneAnglesQuantized, cv::Mat &sceneMagnitudes, cv::Point &edge) {
    for (int y = -criteria.patchOffset; y <= criteria.patchOffset; ++y) {
        for (int x = -criteria.patchOffset; x <= criteria.patchOffset; ++x) {
            ...
            if (sceneAnglesQuantized.at<uchar>(offsetP) == gradient && sceneMagnitudes.at<float>(offsetP) > criteria.minMagnitude) {
                return 1;
            }
        }
    }
...
int Matcher::testDepth(float scale, float diameter, float depthMedian, Window &window, cv::Mat &sceneDepth, cv::Point &stable) {
    for (int y = -criteria.patchOffset; y <= criteria.patchOffset; ++y) {
        for (int x = -criteria.patchOffset; x <= criteria.patchOffset; ++x) {
            ...
            if ((sceneDepth.at<ushort>(offsetP) - depthMedian * scale) < (criteria.depthK * diameter * criteria.info.depthScaleFactor)) {
                return 1;
            }
        }
    }
